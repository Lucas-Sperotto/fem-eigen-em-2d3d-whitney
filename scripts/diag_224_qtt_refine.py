#!/usr/bin/env python3
"""
Run a focused Q_tt refinement campaign for HELMVEC3 Figure 13 / Table 10.

Default outputs:
- out/validation/diag_224_qtt_refine.csv
- out/validation/DIAG_224_QTT_REFINE.md
"""
...
DEFAULT_VARIANTS = (
    "baseline",
    "diag_eq141_blend_half_qtt",
    "diag_eq141_eps_mass_qtt",
)
RUNS_SUBDIR = ".diag_224_qtt_refine"
...
def _build_markdown(rows: list[dict[str, str]], commands: dict[tuple[str, str], list[str]]) -> str:
    ...
    lines.extend(
        [
            ...
            "## Summary",
            "",
            "| Variant | Backend | Point | FEM | Analytic | HELMVEC3 | Err A% | Err H% | ez_ratio | rank | eig | physical | dedup | gap Ez->Et | delta err A vs baseline |",
            "| --- | --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |",
        ]
    )
...
def main() -> int:
    ...
    parser.add_argument(
        "--out-csv",
        type=Path,
        default=DEFAULT_OUT_ROOT / "validation" / "diag_224_qtt_refine.csv",
    )
    parser.add_argument(
        "--out-md",
        type=Path,
        default=DEFAULT_OUT_ROOT / "validation" / "DIAG_224_QTT_REFINE.md",
    )
