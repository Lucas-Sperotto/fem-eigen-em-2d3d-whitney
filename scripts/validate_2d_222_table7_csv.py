#!/usr/bin/env python3
"""
Aggregate CSV-based scientific regression for Section 2.2.2 Table 7 (mixed_circle).

Default output:
- out/validation/validation_2d_222_table7.csv
"""
...
TABLE7_TE_MODES = (
    RequiredMode("TE01", "E", "edge", "TE", "TE_m0_p1"),
    RequiredMode("TE11", "E", "edge", "TE", "TE_m1_p1"),
    RequiredMode("TE12", "E", "edge", "TE", "TE_m2_p1"),
    RequiredMode("TE13", "E", "edge", "TE", "TE_m3_p1"),
    RequiredMode("TE20", "E", "edge", "TE", "TE_m0_p2"),
    RequiredMode("TE21", "E", "edge", "TE", "TE_m1_p2"),
    RequiredMode("TE22", "E", "edge", "TE", "TE_m2_p2"),
    RequiredMode("TE23", "E", "edge", "TE", "TE_m3_p2"),
)

TABLE7_H_TE_MODES = tuple(
    RequiredMode(mode.mode, "H", "scalar", "TE", mode.mode_label) for mode in TABLE7_TE_MODES
)

TABLE7_TM_MODES = (
    RequiredMode("TM10", "E", "scalar", "TM", "TM_m0_p1"),
    RequiredMode("TM11", "E", "scalar", "TM", "TM_m1_p1"),
    RequiredMode("TM12", "E", "scalar", "TM", "TM_m2_p1"),
    RequiredMode("TM13", "E", "scalar", "TM", "TM_m3_p1"),
    RequiredMode("TM20", "E", "scalar", "TM", "TM_m0_p2"),
    RequiredMode("TM21", "E", "scalar", "TM", "TM_m1_p2"),
    RequiredMode("TM22", "E", "scalar", "TM", "TM_m2_p2"),
    RequiredMode("TM23", "E", "scalar", "TM", "TM_m3_p2"),
)

TABLE7_H_TM_MODES = tuple(
    RequiredMode(mode.mode, "H", "edge", "TM", mode.mode_label) for mode in TABLE7_TM_MODES
)

CASE_SPECS = (
    CaseSpec(
        case="mixed_circle_table7_E_TE",
        source_rel=Path("helmvec1/circle/csv/mixed_circle_modes.csv"),
        rerun_cmd=("./build/mixed_circle", "--nr", "10", "--nt", "48"),
        required_modes=TABLE7_TE_MODES,
        expected_count=8,
    ),
    CaseSpec(
        case="mixed_circle_table7_H_TE",
        source_rel=Path("helmvec1/circle/csv/mixed_circle_modes.csv"),
        rerun_cmd=("./build/mixed_circle", "--nr", "10", "--nt", "48"),
        required_modes=TABLE7_H_TE_MODES,
        expected_count=8,
    ),
    CaseSpec(
        case="mixed_circle_table7_E_TM",
        source_rel=Path("helmvec1/circle/csv/mixed_circle_modes.csv"),
        rerun_cmd=("./build/mixed_circle", "--nr", "10", "--nt", "48"),
        required_modes=TABLE7_TM_MODES,
        expected_count=8,
    ),
    CaseSpec(
        case="mixed_circle_table7_H_TM",
        source_rel=Path("helmvec1/circle/csv/mixed_circle_modes.csv"),
        rerun_cmd=("./build/mixed_circle", "--nr", "10", "--nt", "48"),
        required_modes=TABLE7_H_TM_MODES,
        expected_count=8,
    ),
)
...
def _fail_with_missing_modes(
    *,
    ...
    lines.append("Re-run mixed_circle to regenerate the canonical CSV, for example:")
...
def _validate_expected_counts(rows: list[dict[str, str]]) -> None:
    ...
    if mismatches:
        lines = ["Aggregated row count mismatch for Section 2.2.2 Table 7:"]
...
def aggregate_cases(out_root: Path, backend: str) -> list[dict[str, str]]:
    ...
    if missing_files:
        lines = ["Missing required CSV input(s) for Section 2.2.2 Table 7 aggregation:"]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Aggregate Section 2.2.2 Table 7 CSV regressions into one CSV.")
    parser.add_argument(
        "--out-root",
        type=Path,
        default=Path("out"),
    ...
    parser.add_argument(
        "--out-csv",
        type=Path,
        default=Path("out/validation/validation_2d_222_table7.csv"),
