#!/usr/bin/env python3
"""
Aggregate CSV-based scientific regression for Section 2.1 (Tables 1, 2 and 3).

Default output:
- out/validation/validation_2d_21.csv
"""
...
CASE_SPECS = (
    CaseSpec(
        ...
        required_modes=(
            ...
            RequiredMode("TE12", "TE", "TE_m1_n2"),
            RequiredMode("TM12", "TM", "TM_m1_n2"),
            RequiredMode("TE21", "TE", "TE_m2_n1"),
            RequiredMode("TM21", "TM", "TM_m2_n1"),
        ),
        required_columns=("n",),
        expected_count=9,
    ),
    CaseSpec(
        ...
        required_modes=(
            ...
            RequiredMode("TE12", "TE", "TE_m1_p2"),
            RequiredMode("TE21", "TE", "TE_m2_p1"),
            RequiredMode("TE22", "TE", "TE_m2_p2"),
            RequiredMode("TM01", "TM", "TM_m0_p1"),
            RequiredMode("TM11", "TM", "TM_m1_p1"),
            RequiredMode("TM12", "TM", "TM_m1_p2"),
            RequiredMode("TM21", "TM", "TM_m2_p1"),
...
def _validate_expected_counts(rows: list[dict[str, str]]) -> None:
    ...
    expected_case_counts = {case.case: case.expected_count for case in CASE_SPECS}
    ...
    unexpected_cases = sorted(set(case_counts) - set(expected_case_counts))
    for case in unexpected_cases:
        mismatches.append(f"- unexpected case in aggregated rows: {case} ({case_counts[case]} row(s))")

    if mismatches:
        lines = ["Aggregated row count mismatch for Section 2.1:"]
...
def aggregate_cases(out_root: Path, backend: str) -> list[dict[str, str]]:
    ...
    if missing_files:
        lines = ["Missing required CSV input(s) for Section 2.1 aggregation:"]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Aggregate Section 2.1 CSV regressions into one CSV.")
    parser.add_argument("--out-root", type=Path, default=Path("out"), help="Root output directory containing helm10 case folders.")
    parser.add_argument(
        "--out-csv",
        type=Path,
        default=Path("out/validation/validation_2d_21.csv"),
