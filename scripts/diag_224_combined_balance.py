#!/usr/bin/env python3
"""
Run a combined-balance diagnostic campaign for HELMVEC3 Figure 13 / Table 10.

Default outputs:
- out/validation/diag_224_combined_balance.csv
- out/validation/DIAG_224_COMBINED_BALANCE.md
"""
...
DEFAULT_VARIANTS = (
    ...
    "diag_scale_qzz_half",
    "diag_eq141_eps_mass_qtt_plus_pzz_double",
    "diag_eq141_eps_mass_qtt_plus_qzz_half",
)
RUNS_SUBDIR = ".diag_224_combined_balance"
...
def _load_summary_rows(
    ...
    rows.sort(key=lambda row: row["priority_point"])
    return rows, cmd
...
def _baseline_lookup(rows: list[dict[str, str]]) -> dict[str, dict[str, str]]:
    return {row["priority_point"]: row for row in rows if row["variant"] == "baseline"}


def _build_markdown(rows: list[dict[str, str]], commands: dict[str, list[str]]) -> str:
    baseline = _baseline_lookup(rows)
    lines = [
        "# DIAG 224 Combined Balance",
        "",
    ...
    lines.extend(
        [
            ...
            "## Summary",
            "",
            "| Variant | Point | FEM | Analytic | HELMVEC3 | Err A% | Err H% | ez_ratio | rank | eig | physical | dedup | gap Ez->Et | delta err A vs baseline |",
            "| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |",
        ]
    )
    ...
    for variant in DEFAULT_VARIANTS:
        for row in [r for r in rows if r["variant"] == variant]:
            base = baseline[row["priority_point"]]
            delta = float(row["err_primary_pct"]) - float(base["err_primary_pct"])
            lines.append(
                "| "
                + " | ".join(
                    [
...
def main() -> int:
    ...
    parser.add_argument(
        "--out-csv",
        type=Path,
        default=DEFAULT_OUT_ROOT / "validation" / "diag_224_combined_balance.csv",
    )
    parser.add_argument(
        "--out-md",
        type=Path,
        default=DEFAULT_OUT_ROOT / "validation" / "DIAG_224_COMBINED_BALANCE.md",
    )
    ...
    for variant in DEFAULT_VARIANTS:
        variant_rows, cmd = _load_summary_rows(
            build_bin=build_bin,
            out_root=out_root,
            variant=variant,
            nx=args.nx,
    ...
    rows.sort(key=lambda row: (row["variant"], row["priority_point"]))
