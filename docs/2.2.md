...
$$
\frac{1}{\mu_r} \iint_\Delta \sum_{m=1}^{3} (\nabla_t \times \mathbf{W}_{tm}) \cdot (\Delta_e \times \mathbf{W}_{tn}) e_{tm} ds = k_c^2 \varepsilon_r \iint_\Delta \sum_{m=1}^{3} (\mathbf{W}_{tm} \cdot \mathbf{W}_{tn}) e_{tm} ds \quad (n = 1, 2, 3) 
$$
