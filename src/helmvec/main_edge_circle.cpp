...
int main(int argc, char **argv)
{
    ...
    {
        os << "Uso: ./edge_circle [nr nt [nmodos]] [--backend closed-form|gauss|efgmi]"
    ...
    {
        std::cerr << "Erro: nao misture aliases nomeados principais com argumentos posicionais principais em edge_circle; escolha apenas um estilo de chamada.\n";
        print_usage(std::cerr);
        return 2;
    }
    ...
    else
    {
        ...
        {
            std::cerr << "Erro: edge_circle exige ambos nr e nt quando usa argumentos posicionais; use [nr nt [nmodos]].\n";
            print_usage(std::cerr);
            return 2;
        }
        if (cli.positionals.size() > 3)
        {
            std::cerr << "Erro: argumentos posicionais em excesso em edge_circle; use no maximo [nr nt [nmodos]].\n";
            print_usage(std::cerr);
            return 2;
        }
        try
        {
            ...
            {
                nr = helmvec::parse_positive_edge_cli_int(cli.positionals[0], "nr");
                nt = helmvec::parse_positive_edge_cli_int(cli.positionals[1], "nt");
            }
            if (cli.positionals.size() >= 3)
            {
        ...
        {
            std::cerr << "Erro ao interpretar argumentos posicionais de edge_circle: "
                      << e.what() << "\n";
            print_usage(std::cerr);
            return 2;
        }
    ...
    if (!has_named_polar_args && !cli.positionals.empty())
    {
        std::cerr << "Aviso: os argumentos posicionais principais de edge_circle continuam aceitos por compatibilidade, mas estao deprecated; prefira --nr/--nt/--nmodos.\n";
    }
