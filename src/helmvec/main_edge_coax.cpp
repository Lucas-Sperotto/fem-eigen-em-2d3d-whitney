...
int main(int argc, char **argv)
{
    ...
    else
    {
        ...
        {
            std::cerr << "Erro: edge_coax exige ambos nr e nt quando usa argumentos posicionais; use [nr nt [nmodos]].\n";
            print_usage(std::cerr);
            return 2;
        }
        if (cli.positionals.size() > 3)
        {
            std::cerr << "Erro: argumentos posicionais em excesso em edge_coax; use no maximo [nr nt [nmodos]].\n";
            print_usage(std::cerr);
            return 2;
        }
        try
        ...
        {
            std::cerr << "Erro ao interpretar argumentos posicionais de edge_coax: "
                      << e.what() << "\n";
            print_usage(std::cerr);
            return 2;
        }
