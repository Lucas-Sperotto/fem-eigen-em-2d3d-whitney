...
std::string coax_mode_label(const std::string &family, int m, int p)
{
...
int main(int argc, char **argv)
{
    ...
    {
        os << "Uso: ./helm10_coax [nr nt [nmodos]] [--backend closed-form|gauss|efgmi]"
    ...
    const std::string modes_csv_path = output_paths::file_in(csv_dir, "helm10_coax_modes.csv");
    std::ofstream modes_csv(modes_csv_path);
    if (!modes_csv)
    {
        std::cerr << "Erro ao abrir CSV de modos do caso coaxial.\n";
        return 3;
    }
    modes_csv << std::setprecision(16);
    modes_csv << "family,longitudinal_label,mode_label,positive_rank,eig_index,m,p,r1_m,r2_m,"
              << "freq_hz,eps_r,mu_r,k_medium,kc_fem,kc_ana,beta,ztm,below_cutoff,"
              << "error_percent,rho_abs,field_status,fields_csv_file\n";
    ...
    for (int i = 0; i < (int)te.w.size() && exported_te < export_modes; ++i)
    {
        ...
        const auto field_result = helm10::field_reconstruction::reconstruct_transverse_fields(
            ...
            cli.backend,
            false);
        const std::string label = coax_mode_label("TE", id.m, id.p);
        std::ostringstream fields_name;
        fields_name << "helm10_coax_fields_" << label
        ...
        if (exported_te == 1)
            write_mode(sys_te, te, ScalarBC::TE_Neumann, i, "te_coax_sv.vtk");
    }
    ...
    for (int i = 0; i < (int)tm.w.size() && exported_tm < export_modes; ++i)
    {
        ...
        const auto field_result = helm10::field_reconstruction::reconstruct_transverse_fields(
            ...
            cli.backend,
            false);
        const std::string label = coax_mode_label("TM", id.m, id.p);
        std::ostringstream fields_name;
        fields_name << "helm10_coax_fields_" << label
        ...
        if (exported_tm == 1)
            write_mode(sys_tm, tm, ScalarBC::TM_Dirichlet, i, "tm_coax_sv.vtk");
    }
    ...
    timing_record.case_label = "helm10_coax";
