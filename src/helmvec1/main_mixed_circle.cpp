...
/*****************************************************************************/
/* Arquivo: src/helmvec1/main_mixed_circle.cpp                                */
...
#include "mixed_circle_edge_match_wrapper.hpp"
...
void print_circle_matched_table(
    const std::string &title,
    const std::vector<CircleMatchedRow> &rows,
    ...
    int max_rows = 8)
{
    ...
    int shown = 0;
    for (const CircleMatchedRow &row : rows)
    {
        if (shown >= max_rows)
            break;
...
void append_circle_mode_records(
    ...
    std::vector<CircleMatchedRow> &matched_rows)
{
    for (size_t idx = 0; idx < modes.size(); ++idx)
    {
        ...
        rec.r_m = helmvec1_output::format_float(r);
        rec.kc_fem = helmvec1_output::format_float(mode.kc);
        rec.kc_r_fem = helmvec1_output::format_float(mode.kc * r);
        ...
        if (use_edge_block)
        {
            ...
            const CircEdgeModeID id = is_te_family
                                          ? match_circle_edge_mode_by_mass_correlation_TE(
                                                mesh, r, sys.edge.T, sys.edge.ed, block_zcol, 0, 6, 6)
                                          : match_circle_edge_mode_by_mass_correlation_TM(
                                                mesh, r, sys.edge.T, sys.edge.ed, block_zcol, 0, 6, 6);
            rec.m = std::to_string(id.m);
            rec.p = std::to_string(id.p);
            rec.kc_ana = helmvec1_output::format_float(id.kc_ana);
            rec.kc_r_ana = helmvec1_output::format_float(id.kc_ana * r);
        ...
        else
        {
            const std::vector<double> block_zcol =
                helmvec1_match::extract_scalar_block_mode_as_column_major(
                    mixed_zcol, sys.nt, sys.nz, mode.eig_index);
            const CircModeID id = match_circle_mode_by_mass_correlation(
                mesh,
                r,
                sys.scal,
                block_zcol,
...
int main(int argc, char **argv)
{
    ...
    if (!spectral_csv::write_symmetric_problem_exports(dirs.linop, "mixed_circle_E", sys_e.S, sys_e.T, res_e) ||
        !spectral_csv::write_dense_crs_csv(dirs.linop / "mixed_circle_E_St_crs.csv", sys_e.St) ||
        !spectral_csv::write_dense_crs_csv(dirs.linop / "mixed_circle_E_Tt_crs.csv", sys_e.Tt) ||
        !spectral_csv::write_dense_crs_csv(dirs.linop / "mixed_circle_E_Sz_crs.csv", sys_e.Sz) ||
        !spectral_csv::write_dense_crs_csv(dirs.linop / "mixed_circle_E_Tz_crs.csv", sys_e.Tz))
    {
    ...
    std::cout << "\n[E-formulation | circle]\n";
    std::vector<CircleMatchedRow> matched_te_edge_e;
    std::vector<CircleMatchedRow> matched_tm_scalar_e;
    append_circle_mode_records(
        "E", "edge", "Et", "TE",
        modes_te_edge_e, "circle", dirs, mesh, sys_e, res_e.Zcol, true, true, r,
        mode_records, matched_te_edge_e);
    append_circle_mode_records(
        "E", "scalar", "Ez", "TM",
        modes_tm_scalar_e, "circle", dirs, mesh, sys_e, res_e.Zcol, false, false, r,
        mode_records, matched_tm_scalar_e);
    print_circle_matched_table("TE (edge block)", matched_te_edge_e, lambda_filter);
    print_circle_matched_table("TM (scalar block)", matched_tm_scalar_e, lambda_filter);
    ...
    if (!spectral_csv::write_symmetric_problem_exports(dirs.linop, "mixed_circle_H", sys_h.S, sys_h.T, res_h) ||
        !spectral_csv::write_dense_crs_csv(dirs.linop / "mixed_circle_H_St_crs.csv", sys_h.St) ||
        !spectral_csv::write_dense_crs_csv(dirs.linop / "mixed_circle_H_Tt_crs.csv", sys_h.Tt) ||
        !spectral_csv::write_dense_crs_csv(dirs.linop / "mixed_circle_H_Sz_crs.csv", sys_h.Sz) ||
        !spectral_csv::write_dense_crs_csv(dirs.linop / "mixed_circle_H_Tz_crs.csv", sys_h.Tz))
    {
    ...
    std::cout << "\n[H-formulation | circle]\n";
    std::vector<CircleMatchedRow> matched_tm_edge_h;
    std::vector<CircleMatchedRow> matched_te_scalar_h;
    append_circle_mode_records(
        "H", "edge", "Ht", "TM",
        modes_tm_edge_h, "circle", dirs, mesh, sys_h, res_h.Zcol, true, false, r,
        mode_records, matched_tm_edge_h);
    append_circle_mode_records(
        "H", "scalar", "Hz", "TE",
        modes_te_scalar_h, "circle", dirs, mesh, sys_h, res_h.Zcol, false, true, r,
        mode_records, matched_te_scalar_h);
    print_circle_matched_table("TM (edge block)", matched_tm_edge_h, lambda_filter);
    print_circle_matched_table("TE (scalar block)", matched_te_scalar_h, lambda_filter);
    perf_h.post_ms = stage.elapsed_ms();
    perf.post_ms += perf_h.post_ms;

    perf.total_ms = total_watch.elapsed_ms();
    timing::print_breakdown("mixed_circle", perf);

    const std::string modes_csv_path =
        output_paths::file_in(dirs.csv, "mixed_circle_modes.csv");
    if (helmvec1_output::write_modes_csv(dirs.csv / "mixed_circle_modes.csv", mode_records))
        std::cout << "Saved: " << modes_csv_path << "\n";
    else
        std::cerr << "Aviso: falha ao escrever " << modes_csv_path << "\n";

    run_timing_mixed_csv::Record timing_record;
    timing_record.case_label = "mixed_circle";
    ...
    timing_record.r_m = helmvec1_output::format_float(r);
